...
<b-container>
  ...
  <b-card header="Комплектация" class="mb-4 shadow-sm">
    <div class="mb-2">
      <b-button variant="info" class="mr-3" @click="onSelectAllEquips">{{ selectionBtnTextEquip }}</b-button>
...
export default class RealtyTypeRelationship extends Mixins<TableStateController>(TableStateController) {
  ...
  get selectionBtnTextEquip(): string {
    return this.selectedAllEquipsRows ? 'Снять выделение' : 'Выбрать все'
  }
  ...
  updateTableData(): Promise<AxiosResponse<responseWithPaginator<Realty>>> {
    return Realty.getList({ ...this.tableOptionsCleared, types: [ this.$route.params.id ] })
        .then(response => {
